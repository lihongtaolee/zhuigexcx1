	...
	<view class="content">
		<view class="zhuige-category">
			...
			<view class="zhuige-category-nav">
				<view v-for="(cat,index) in cats" :key="index" :class="cur_cat_id==cat.id ? 'active' : ''" @click="clickTopCat(cat.id)">
					{{cat.name}}
				</view>
			</view>
			...
			<template v-for="(cat,index) in cats">
				<view v-if="cur_cat_id==cat.id" :key="index" class="zhuige-category-list">
					...
					<view v-for="(subcat,inx) in cat.subs" :key="inx">
						<view @click="clickLink('/pages/shop/list?cat_id=' + subcat.id + '&title=' + subcat.name)" class="zhuige-category-title">
							{{subcat.name}}
						</view>

						<view class="zhuige-category-group">
							<view v-for="(goods,ginx) in subcat.list" :key="ginx" class="zhuige-category-block" @click="clickLink('/pages/shop/detail?goods_id=' + goods.id)">
